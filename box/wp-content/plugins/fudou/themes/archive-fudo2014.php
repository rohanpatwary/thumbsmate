 ...
 *
 * Template Name: archive-fudo2014.php
 * 
 ...
 */
	...
	//路線・県ID
		...
		$cho_data = isset($_GET['cho']) ? $_GET['cho'] : '';
		if( $cho_data != ''){
			$cho_data = esc_sql(esc_attr( $cho_data ));
			$cho_data = trim( $cho_data );
			if( $cho_data == '0' ); $cho_data = '';
			$cho_data = str_replace(";","",$cho_data);
			$cho_data = str_replace(",","",$cho_data);
			$cho_data = str_replace("'","",$cho_data);
			$cho_data = str_replace("\\","",$cho_data);
		}
	...
	//カテゴリ
		...
		$bukken_slug_data = esc_sql(esc_attr( $bukken ));
		$taxonomy_name = '';
	...
	//投稿タグ
		if($bukken_slug_data == ''){
			...
			$bukken_slug_data = esc_sql(esc_attr( $bukken_tag ));
	...
	//種別
	//	$bukken_shubetsu = esc_attr( stripslashes($_GET['shu']));
	...
	//複数種別
		...
		if (is_array($bukken_shubetsu)) {
			...
			//土地判定
			//if( ($bukken_shubetsu > 1100 && $bukken_shubetsu < 1300) || $bukken_shubetsu == 3212  ){
			//	$is_tochi = true;
			//}
	...
	$rosen_eki = isset($_GET['re']) ? $_GET['re'] : '';	//路線駅
	$ksik_id = isset($_GET['ksik']) ? $_GET['ksik'] : '';	//県市区
	if($bukken_slug_data == "jsearch"){
		...
		//間取り
		$madori_id = isset($_GET['mad']) ? $_GET['mad'] : '';
		$madori_name = '';
		if(!empty($madori_id)) {
			$madori_name = '間取り';
	...
	//<title></title> <h2></h2>設定
	add_action('wp_title', 'add_post_type_wp_title');
	...
	if($bukken_slug_data=="jsearch"){
		...
		if( $shu_data != '' && ( ($ken_id !='' && $ken_id != 0) || ($ros_id !='' && $ros_id != 0)  || $ksik_data !='' || $eki_data != ''  ) ){
			...
			//名称
			if( $cho_data != '' ){
				$sql .=  " INNER JOIN $wpdb->postmeta AS PM_10C ON P.ID = PM_10C.post_id) ";
			}else{
				$sql .=  " )";	
			}
			...
			//名称
			if( $cho_data !='' ){
				$sql .=  " AND PM_10C.meta_key='shozaichimeisho' AND trim(PM_10C.meta_value) = '".$cho_data."'";
			}
		...
		if($next_sql){
			...
			if($mel_data > 0 || $meh_data > 0 || $bukken_sort_data == "tatemonomenseki" ){
				...
				if( ($bukken_shubetsu > 1100 && $bukken_shubetsu < 1200) || $bukken_shubetsu == 3212 ){
	...
	//ソート・ページナビ
		...
		if($metas_co != 0 ){
			...
			//	if($ken_id != '') $ken_id = intval($ken_id);
				...
				$add_url .= '&amp;ken='. $ken_id;
				$add_url .= '&amp;sik='. $sik_id;
				if( $cho_data != '' ) $add_url .= '&amp;cho='. $cho_data;
...
?>
	<section id="primary" class="content-area">
		<div id="content" class="site-content" role="main">
			...
			<?php if( $joken_url !='' ) { ?>
				<h1 class="page-title"><a href="<?php echo $joken_url;?>"><?php wp_title( '|', true, 'right' ); ?></a></h1>
			<?php  }else{  ?>
 ...
 * @return text
 */
function f_page_navi( $fw_record_count,$fw_page_size,$fw_page_count,$bukken_sort,$bukken_order,$bukken_sort,$s,$joken_url ){
